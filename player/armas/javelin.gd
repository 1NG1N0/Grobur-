...
var spr_jav_reg = preload("res://player/armas/pixil-frame-0 (37).png")
var spr_jav_atk = preload("res://player/armas/pixil-frame-0 (38).png")
...
func _physics_process(delta):
	if target_array.size() > 0:
		...
		if abs(distance_dif.x) > 500 or abs(distance_dif.y) > 500:
			return_speed = 100.0
		position += player_angle*return_speed
